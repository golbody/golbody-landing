...
async function buildAll() {
  ...
  await esbuild({
    entryPoints: [
      ...
      path.resolve(artifactDir, "src/api.ts"),
    ],
    platform: "node",
    bundle: true,
    format: "esm",
...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  // outside the api/ directory, so we bundle the API entrypoint directly here.
  await esbuild({
    entryPoints: {
      index: path.resolve(artifactDir, "src/api.ts"),
    },
    platform: "node",
    bundle: true,
    format: "esm",
    ...
    sourcemap: "linked",
    plugins: [
      // Skip pino plugin for Vercel — bundling as a single serverless entrypoint
    ],
    banner: {
      js: `import { createRequire as __bannerCrReq } from 'node:module';
import __bannerPath from 'node:path';
